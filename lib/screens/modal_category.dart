                      ...
                      if (state.category.isNotEmpty) {
                        return GridView.count(
                          ...
                          children: state.category
                              .map((categoryItem) =>
                                  ExpenseCategoryButton(category: categoryItem, changeScreen: widget.changeScreen))
                              .toList(),
                        );
                      }
                    }
                    ...
                    if (state is IncomeCategoryLoaded) {
                      if (state.category.isNotEmpty) {
                        return GridView.count(
                          ...
                          children: state.category
                              .map((categoryItem) =>
                                  IncomeCategoryButton(category: categoryItem, changeScreen: widget.changeScreen))
                              .toList(),
                        );
                      }
                    }
                    ...
                    if (state is GoalLoaded) {
                      if (state.goal.isNotEmpty) {
                        return GridView.count(
                          ...
                          crossAxisSpacing: 1,
                          children: state.goal
                              .map((goalItem) => GoalButton(goal: goalItem, changeScreen: widget.changeScreen))
                              .toList(),
                        );
                      }
                    }
        ...
        children: [
          Padding(
            ...
            child: RichText(
              ...
              text: TextSpan(style: const TextStyle(color: Colors.white, fontSize: 12), text: goal.name),
            ),
          ),
          CircleAvatar(
...
class ExpenseCategoryButton extends StatelessWidget {
  ...
  const ExpenseCategoryButton({required this.category, this.changeScreen, Key? key}) : super(key: key);
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: RichText(
              overflow: TextOverflow.ellipsis,
              text: TextSpan(style: const TextStyle(color: Colors.white, fontSize: 12), text: category.name),
            ),
          ),
          CircleAvatar(
            backgroundColor: Colors.grey.shade200,
...
class IncomeCategoryButton extends StatelessWidget {
  ...
  const IncomeCategoryButton({required this.category, this.changeScreen, Key? key}) : super(key: key);
